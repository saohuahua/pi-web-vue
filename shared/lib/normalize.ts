// Ported from pi-web lib/normalize.ts — https://github.com/agegr/pi-web (MIT)
// 文件里的 toolCall 字段名是 id name arguments 而运行时与前端类型用 toolCallId toolName input
// 读文件与收流式事件两条路径都必须过这个函数 否则工具卡片时有时无
...
function normalizeAssistantToolCalls(
  ...
  // 非 assistant 角色 原样返回 只有 assistant 消息需要 toolCall 字段归一化
