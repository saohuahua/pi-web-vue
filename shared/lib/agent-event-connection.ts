// Ported from pi-web lib/agent-event-connection.ts — https://github.com/agegr/pi-web (MIT)
// 封装 EventSource 的连接管理
// 等 connected 事件才算就绪 带超时 断线指数重连 startup_error 停止重试
// 纯类 无 React 依赖 Vue 中原样可用
...
export class AgentEventConnection {
  ...
  async ensureConnected(sessionId: string): Promise<void> {
    while (true) {
      ...
      // once ready 的 EventSource 可能一直停在 CONNECTING 不报错 主动丢弃重连
