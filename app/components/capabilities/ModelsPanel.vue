<template>
  <section class="cap-split" aria-labelledby="models-title">
    <aside class="cap-index">
      ...
      <div v-if="models.loading && !models.modelList.length" class="cap-loading-list" aria-label="正在加载模型">
        <span v-for="index in 5" :key="index"></span>
      </div>
      <p v-else-if="!models.modelList.length" class="cap-empty">选择项目后可查看本机已配置模型</p>
      ...
      <template v-if="selected">
        <header class="cap-detail-head">
          <div>
            <div class="cap-detail-title-row">
              ...
              <span v-if="isCurrent(selected)" class="cap-status active">当前会话</span>
              <span v-else-if="isDefault(selected)" class="cap-status">默认模型</span>
            </div>
            <p class="cap-mono">{{ selected.provider }} / {{ selected.id }}</p>
          </div>
          <div class="cap-action-row">
            <button
              class="cap-secondary-button"
              type="button"
              :disabled="isDefault(selected) || savingDefault"
              @click="setDefault"
            >{{ defaultLabel }}</button>
            <button
              class="cap-primary-button"
              type="button"
        ...
        <div class="cap-section">
          ...
          <div class="cap-tag-list">
            <span v-for="kind in selected.input" :key="kind" class="cap-tag">{{ inputLabel(kind) }}</span>
        ...
        <div class="cap-section">
          ...
          <div v-if="thinkingLevels.length" class="cap-tag-list">
            <span v-for="level in thinkingLevels" :key="level" class="cap-tag mono">{{ level }}</span>
          </div>
...
const modelError = computed(() => {
  if (/auth\.json\.lock|EPERM/i.test(models.modelError)) return "模型配置正被其他 pi 进程占用 请稍后刷新";
...
const thinkingLevels = computed(() => selected.value
  ? models.thinkingLevels[keyOf(selected.value)] ?? []
  : []);
...
function keyOf(model: Pick<ModelListEntry, "provider" | "id">) {
  return `${model.provider}:${model.id}`;
}

function isCurrent(model: Pick<ModelListEntry, "provider" | "id">) {
  return chat.model?.provider === model.provider && chat.model.id === model.id;
}

function isDefault(model: Pick<ModelListEntry, "provider" | "id">) {
  return models.defaultModel?.provider === model.provider && models.defaultModel.modelId === model.id;
}
