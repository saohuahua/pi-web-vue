...
进度：`[x]` 2026-09-10 完成。四项验收均通过（含临时多 worktree git 仓库实测）。额外修复：新建空会话首条消息前 SDK 不落盘 `.jsonl`，详情路由原直接读文件抛 ENOENT，现用 wrapper 内存态兜底；空会话不进列表与 pi-web 行为一致。工作区状态持久化到 localStorage，打开会话时工作区跟随其项目与 worktree。
...
进度：`[x]` 2026-09-10 完成。安全验收含真 junction 逃逸（MSYS `ln -s` 建的是拷贝不是链接 需用 node symlinkSync junction 类型测试）全部 403。file-index 沿用 pi-web 的 git ls-files 语义 未跟踪新文件不出现在搜索 但目录树可见。
...
进度：`[x]` 2026-09-10 完成。重命名与自动标题经用户实际操作验证（中文对话生成中文标题 未串会话）。session-title 的 generateSessionTitle 直接接收 Agent 类型 路由处对 AgentSessionInner.agent 做结构断言。
...
进度：`[x]` 2026-09-10 完成。模型下拉含 1362 个模型 思考等级来自 getSupportedThinkingLevels 无推理能力的模型隐藏整块。发现并处理：SDK 对纯文本模型静默丢图（glm-5.3 input 仅 text 实测图片不落盘不达模型）前端按模型能力阻止并提示。压缩失败路径实测（SDK 拒绝过小会话 composer 不卡 错误可读）成功路径依赖 compaction_ 事件流 Step A 已实现 留大会话日常验证。/ 命令只含 get_commands 的模板与技能 未实现 pi CLI 内置命令。
...
进度：`[x]` 2026-09-10 完成。成本 0（无价格数据或免费模型）显示 -- 与 pi-web 语义一致 不估算。窄屏图标化留给 SA-8 一并处理。模型名从顶栏移除（选择器在 composer 工具行）顶栏专注会话操作与用量。
...
进度：`[x]` 2026-09-10 完成。技能开关往返实测 md5 与原文件一致。设置含三档主题（深色为新增 token 覆盖 带实测截图）与完成提示音。模型页只读列表截 200 条 完整切换在输入框选择器。
...
进度：`[x]` 2026-09-10 完成。桌面布局左栏上会话下文件树 顶栏与 composer 固定。移动端抽屉 顶栏数值折叠（仅保留警告态 context）实测无横向溢出。自动化补齐 store 测试：最后选择胜出 统计不串会话 模型与思考切换失败回滚 重命名打正确 id 并强刷 自动标题防重复点击；store 文件为此补显式 vue 导入。烟测覆盖两项目切换 临时多 worktree 仓库 带图消息 旧 .jsonl 会话。控制台零错误 107 测试 typecheck 全过。测试环境注意：预览面板窗口失焦时页面停止绘制 transition 冻结 computed style 读到起始值 非代码 bug。
...
- [x] SA-1 底层契约、命令与安全边界
- [x] SA-2 项目、Git worktree 与会话工作区
- [x] SA-3 文件树与安全文件查看
- [x] SA-4 会话管理、自动标题与项目内搜索
- [x] SA-5 模型、思考、图片与压缩
- [x] SA-6 顶栏基础操作和使用量
- [x] SA-7 模型、技能与设置面板
- [x] SA-8 UI 收口与全链路验收

Step A+ 全部完成 2026-09-10 下一步进入 Step B 的 06-history-branching.md 会话内分支
